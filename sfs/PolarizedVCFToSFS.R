...
for (pop in pops){
  indivs = read.delim("~/Documents/TigerProject/Rscripts/IndivMetaData/18sAnd6s_annotated.txt") %>%
  ...
  for (i in seq_along(fnames)){
    vcf = fread(file=paste0("~/Documents/TigerProject/Rscripts/sfs/Unfolded/", fnames[i])) %>% 
  ...
  write.table(summaryInfo, file = paste(pop, "_SFS_allChroms_SummaryFile_N18.txt", sep = ""), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
