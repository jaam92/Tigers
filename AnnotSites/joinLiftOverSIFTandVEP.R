...
###Start the work
#read in coords for tiger and felcat8 from bernard's liftover
tigerFelCatCoors = fread("tiger_to_FC8.sortedBasedOnTiger.putCatChromsOnly.gz", col.names = c("chrom","tigerStart", "tigerEnd", "chromfel8", "fel8Start", "fel8End", "hash"))
...
vep = fread(cmd=skipHash) %>%
  ...
  mutate(IMPACT = str_replace_all(IMPACT, c("0" = "LOW", "1" = "MODERATE", "2" = "HIGH")), #change the impact column back to original annot
         REF_ALT = as.character("REF/ALT"), #column to split and keep track of allele
         tigerPos = tigerFelCatCoors$tigerEnd[match(fel8pos, tigerFelCatCoors$fel8End)], #add tiger vcf pos back
         recodeConsequence = str_replace_all(Consequence, c("missense_variant,splice_region_variant" = "NS", "splice_donor_variant,missense_variant" = "NS",  "splice_region_variant,synonymous_variant" = "SY", "splice_region_variant,synonymous_variant" = "SY", "stop_retained_variant" = "SY",  "stop_gained,splice_region_variant" = "LOF", "stop_lost" = "LOF", "start_lost" = "LOF", "start_lost,synonymous_variant" = "LOF", "stop_gained,start_lost" = "LOF", "missense_variant" = "NS", "stop_gained" = "LOF", "synonymous_variant" = "SY"))) %>% #recode to match script and categorize as either NS, SY, LOF
...
finalDF = vep %>%
  ...
  select(chrom,fel8pos,tigerPos,REF_ALT_alleles,Allele,SIFT_score,SIFT_Consequence,Consequence,recodeConsequence,IMPACT,Amino_acids,STRAND,Transcript_id,Gene_id,SYMBOL_SOURCE, SYMBOL,Feature_type) %>% #keep only cols of interest
